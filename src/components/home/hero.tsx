...
function GlassPanel({ children }: { children: React.ReactNode }) {
  ...
  const translateZ = useTransform(springX, [-40, 40], [0, 12])
  ...
  const onMove = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    mx.set((clientX - cx) * 0.35)
    my.set((clientY - cy) * 0.35)
  }
...
export function Hero() {
  return (
    <section className="relative min-h-[100svh] flex items-center justify-center overflow-hidden glass-mesh [perspective:1200px]">
      <HeroCanvas />

      <div className="absolute inset-0 pointer-events-none z-[1]" aria-hidden>
        <motion.div
          className="absolute top-[12%] left-[8%] h-44 w-44 rounded-full bg-blush/30 blur-3xl"
          animate={{ y: [0, 28, 0], x: [0, 18, 0] }}
          transition={{ duration: 9, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute bottom-[18%] right-[6%] h-52 w-52 rounded-full bg-gold/20 blur-3xl"
          animate={{ y: [0, -22, 0], x: [0, -14, 0] }}
          transition={{ duration: 11, repeat: Infinity, ease: "easeInOut" }}
        />
      </div>

      <div className="relative z-10 mx-auto max-w-xl w-full px-4 sm:px-5 pt-20 pb-12 pointer-events-none [transform-style:preserve-3d]">
        <GlassPanel>
          <motion.div
